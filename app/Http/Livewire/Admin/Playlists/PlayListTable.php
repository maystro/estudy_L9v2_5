...
class PlayListTable extends Component
{
                ...
                if ($column->filtered) {
                    if (isset($this->filter[$column->name])) {
                        if ($column->type == 'string' && $this->filter[$column->name] != '') {
                            $this->filter_info[] = ['k' => $column->label, 'v' => $this->filter[$column->name]];
    ...
    {
        $this->dataBuilder($this->sortColumn, $this->sortDirection);
        return view('livewire.admin.playlists.play-list-table',
            [
                'data' => $this->config,
    ...
    {
        $this->emitTo('admin.playlists.create-playlist', 'newPlaylist', null);
    }
