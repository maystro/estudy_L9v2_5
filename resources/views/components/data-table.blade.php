<div class="card">
    ...
    <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="mb-0"><i class='bx bx-list-ul bx-lg'></i>
            {{$config->page_title}}
        </h5>

        <div class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
            @if($config->features->enable_create)
                @can('create')
                    <button type="button" class="btn btn-primary me-2"
                            data-bs-toggle="modal"
                            data-bs-target="{{$createFormModal}}">
                        <span class="tf-icons bx bx-plus me-0 me-sm-0"></span>
                    </button>
                @endcan
            @endif
            @if($config->features->enable_print)
                <button type="button"
                        data-bs-toggle="tooltip" data-bs-offset="0,4" data-bs-placement="top"
                        title="طباعة ما تم اختياره"
            ...
            @endif
            @if($config->features->enable_reset_sort)
    ...
    {{--         Generate Filter           --}}
    @if($config->features->enable_filter)
                    ...
                    @if($column->filtered)
                        <div class="col">
                            @switch($column->type)
                                ...
                                @case('datetime')
                                <label>تصفية بـ: {{$column->label}}</label>
                                <input type="text" class="form-control flatpickr-date enFont"
                                       wire:model="filter.{{$column->nameAs}}" placeholder="YYYY-MM-DD"
                                       id="flatpickr-date"/>
                                @break
                                {{---------- NUMERIC ----------}}
                                @case('numeric')
                                <label>تصفية بـ: {{$column->label}}</label>
                                <input type="number" class="form-control enFont"
                                       wire:model="filter.{{$column->nameAs}}" placeholder="1"/>
                                @break;
                            @endswitch
                        </div>
                    @endif
    ...
    @if(count($selectedItems)>0)
        <div class="card-header mt-1 mb-0 pt-0 pb-0">
            <nav class="navbar navbar-expand-lg navbar-dark rounded bg-label-secondary mb-2">
                                    ...
                                    @foreach($this->navMenu as $key=>$menuItem)
                                        ...
                                        @else
                                            <li><a class="dropdown-item"
                                                   wire:click="navMenuItemClicked('{{$menuItem['action']}}')"
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class="table card-table table-hover">
                <thead>
                <tr>
                    @if($config->features->enable_select_all)
                        <td>
                    ...
                    @foreach($config->columns as $column)
                        @if($column->sortable)
                            <th>
                                <span wire:click="sortByColumn('{{$column->name}}')" class="cursor-pointer">
                                    <i class='bx bxs-sort-alt bx-xs'></i>
                                    {!! $this->getHeaderLabel($column) !!}
                                </span>
                            </th>
                        @elseif($column->visible??null)
                            <th>{!! $this->getHeaderLabel($column) !!}</th>
                        @endif
                    @endforeach

                    @if($config->features->enable_data_action)
                ...
                @foreach($config->rows as $row)
                    <tr data-id="{{$row[$config->primaryKey]}}" data-order="{{$row[$config->orderKey]}}">
                        @if($config->features->enable_select)
                            <td>
                                @if($config->features->sortable)
                                    <i class='drag-handle text-muted bx bx-dots-vertical-rounded'></i>
                                @endif
                            ...
                            @if( $column->visible??null )
                                @if($column->clickable??null)
                                    <td class="cursor-pointer"
                                        wire:click="cellClicked('{{$column->name}}','{{$row[$config->primaryKey]}}')">
                                        {!! $this->getCellContent($row, $column, $key) !!}
                                    </td>
                                @else
                                    <td>{!! $this->getCellContent($row, $column, $key) !!}</td>
                                @endif
                            @endif
                        @endforeach
                        ...
                        @if($config->features->enable_data_action)
                            <td class="text-center">
                                @if($config->features->enable_edit)
                                    @can('update')
                                        <button type="button"
                                                data-bs-toggle="modal"
                                                data-bs-target="{{$this->editFormModal}}"
                                                wire:click="actionButtonClicked('edit',{{$row[$config->primaryKey]}})"
                                                class="btn btn-icon me-2 btn-primary">
                                            <span class="tf-icons bx bx-pencil"></span>
                                        </button>
                                    @endcan
                                @endif
                                @if($config->features->enable_delete)
                                    @can('delete')
                                        <button type="button"
                                                wire:click="actionButtonClicked('delete',{{$row[$config->primaryKey]}})"
    ...
    {{--       Generate Pagination         --}}
    @if($config->features->enable_paginate)
